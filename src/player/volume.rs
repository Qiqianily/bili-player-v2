...
impl VolumeManager {
    ...
    // 设置音量大小
    pub fn set_volume(&self, percentage: u32) -> PlayerResult<()> {
        // 验证范围
    ...
    /// 调节音量大小，增加音量
    pub fn increase_volume(&self, step: u32) -> PlayerResult<u32> {
    ...
    /// 调节音量大小，减少音量
    pub fn decrease_volume(&self, step: u32) -> PlayerResult<u32> {
        let current = self.get_volume_percentage();
    ...
    /// 静音
    pub fn toggle_mute(&self) -> PlayerResult<bool> {
        let is_muted = self.is_muted();

        if is_muted {
            // 取消静音，恢复之前的音量
            let previous_volume = self.get_volume_percentage();
            self.apply_volume(Self::percentage_to_gstreamer(previous_volume))?;
    ...
    /// 设置音量
    fn apply_volume(&self, volume: f64) -> PlayerResult<()> {
        if let Some(volume_elem) = self.pipeline.by_name("audio_volume") {
